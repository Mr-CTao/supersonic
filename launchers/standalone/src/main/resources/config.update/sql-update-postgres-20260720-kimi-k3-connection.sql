...
-- The NOT EXISTS guards make the migration idempotent and preserve any administrator-managed
-- connection that already uses the same stable name.
INSERT INTO s2_chat_model (
    name, description, config, created_at, created_by, updated_at, updated_by, admin, viewer, is_open
)
SELECT
    'Kimi-K3',
...
-- K3 capability metadata is inserted separately so the Gateway can enforce real model limits
-- before the first external call. It intentionally references the existing row when the seed
-- connection was already present.
...
SELECT
    ...
    true, true, true, true,
    true, false, true,
    true, 1.0, 'semantic_modeling', true,
