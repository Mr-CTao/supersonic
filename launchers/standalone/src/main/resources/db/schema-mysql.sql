...
CREATE TABLE IF NOT EXISTS `s2_semantic_gap` (
    ...
    `diagnostic_stage` varchar(64) DEFAULT NULL COMMENT '结构化诊断阶段',
    ...
    `error_column` int DEFAULT NULL COMMENT '错误列号',
    `error_token` varchar(128) DEFAULT NULL COMMENT '安全错误token',
    `suggestion` varchar(1000) DEFAULT NULL COMMENT '脱敏修复建议',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `s2_semantic_modeling_draft` (
    ...
    `route_analysis_id` bigint DEFAULT NULL COMMENT '已确认且未过期的语义资产路由分析ID；历史草稿为空',
    `route_action` varchar(32) DEFAULT NULL COMMENT '确认动作：CREATE_NEW或EXTEND_EXISTING',
    `route_target_asset_type` varchar(32) DEFAULT NULL COMMENT '增强动作绑定的目标资产类型',
    `route_target_asset_id` bigint DEFAULT NULL COMMENT '增强动作绑定的目标资产ID',
    `route_target_asset_version` bigint DEFAULT NULL COMMENT '草稿生成时锁定的目标资产版本',
    ...
    UNIQUE KEY `uk_semantic_draft_route_analysis` (`route_analysis_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='AI语义建模结构化草稿主表';

-- AI semantic asset routing snapshots are isolated from formal metadata. JSON text columns are
-- size-limited and schema-validated by the service before persistence.
