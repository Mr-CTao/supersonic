 ...
 * 语义缺口持久化对象。
 *
 * <p>职责说明：映射 `s2_semantic_gap` 表，用于保存 Chat BI 问答失败、负反馈和回退信号的聚合结果。安全说明：SQL、S2SQL、
 * 用户问题和反馈在进入本对象前由服务层截断并做基础脱敏，避免管理端列表暴露长文本或敏感号码。并发说明：本对象不持有共享状态；
 * 同类缺口并发归并由语义缺口服务的分段锁保护。</p>
 */
@Data
