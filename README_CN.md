...
标签市场、标签对象管理、维度/指标“设为标签”等能力默认不在开源构建菜单中展示。二次开发或私有部署如需使用完整标签链路，可以在前端构建或开发启动时显式开启：

```bash
cd webapp/packages/supersonic-fe

# 本地开发：打开标签市场和标签对象管理入口
pnpm dev:os:tag

# 生产构建：生成包含标签入口的开源版前端产物
pnpm build:os:tag
```
...
```bash
SHOW_TAG=true pnpm dev
SHOW_TAG=true pnpm build
```

开启后建议按以下链路验证新库可用性：创建主题域 -> 创建标签对象 -> 创建模型并绑定标签对象 -> 将维度或指标设为标签 -> 进入标签市场查询标签。
